...
switch() {
    ...
    source "$(eval echo ~/.config/quickshell/hyprdotsenv/bin/activate)"
...
main() {
    ...
    echo "  color: $color"
    echo "  noswitch_flag: $noswitch_flag"

    # If type_flag is not set, get it from config
    if [[ -z "$type_flag" ]]; then
        type_flag="$(get_type_from_config)"
        echo "Type flag not set, using from config: $type_flag"
    fi
    ...
    if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" ]]; then
        echo "No image, color, or noswitch flag provided. Prompting for wallpaper."
    ...
    if [[ "$type_flag" == "auto" ]]; then
        if [[ -n "$imgpath" && -f "$imgpath" ]]; then
            ...
            if [[ $valid_detected -eq 1 ]]; then
                ...
                echo "Detected scheme type: $type_flag"
            else
