...
Variants {
    ...
    PanelWindow {
        ...
        // Wallpaper
        property bool wallpaperIsVideo: wallpaperPath.endsWith(".mp4")
        ...
        Component.onCompleted: swwwQueryProcess.running = true
        ...
        Process {
            ...
            stdout: StdioCollector {
                id: swwwQueryOutput
                onStreamFinished: {
                    const output = swwwQueryOutput.text.trim();
                    // swww query output is like: "swww: Displaying image: /path/to/image.png on DP-1"
                    if (output && output.includes("image: ")) {
                        const newPath = output.split("image: ")[1].split(" on ")[0].trim();
                        if (newPath && newPath !== bgRoot.wallpaperPath) {
                            bgRoot.wallpaperPath = newPath;
                        }
                    }
                }
            }
