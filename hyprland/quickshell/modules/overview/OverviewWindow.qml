...
Item { // Window
    ...
    property var iconPath: {
        const guessedIcon = AppSearch.guessIcon(windowData?.class)
        const iconPath = Quickshell.iconPath(guessedIcon, "image-missing")
        console.log(`[OverviewWindow] Window: ${windowData?.class}, Guessed icon: ${guessedIcon}, Path: ${iconPath}`)
        return iconPath
    }
